...
'use server'; // 🔒 Garante que este arquivo NUNCA vaze para o navegador
...
// ======================================================
// METHODS (SCHEDULES, SECTORS, SCALES, ETC.)
// ======================================================
...
export const getEmployeeById = async (tenantId: string, employeeId: string) => {
  try {
    const snap = await tenantRef(tenantId).collection('employees').doc(employeeId).get();
...
export const addEmployee = async (tenantId: string, data: any) => {
  try {
    return await tenantRef(tenantId).collection('employees').add({
      ...data,
      tenantId,
...
export const updateEmployee = async (tenantId: string, employeeId: string, data: any) => {
  try {
    return await tenantRef(tenantId).collection('employees').doc(employeeId).update({
      ...data,
...
export const addPunch = async (tenantId: string, punchData: any) => {
  try {
    return await tenantRef(tenantId).collection('punches').add({
      ...punchData,
      createdAt: new Date().toISOString(),
    });
  } catch (error) {
