...
export default function JustificationsPage() {
    ...
    // 1. Query filtrada por Tenant para isolamento de dados
    const justificationsQuery = useMemoFirebase(() => {
        if (!firestore || !userData?.tenantId) return null;
        return query(
            collection(firestore, 'justifications'), 
    ...
    // 2. Handler de submissão com injeção de metadados do SaaS
    const handleSubmitJustification = async (formData: Omit<Justification, 'id'>) => {
        ...
        try {
            ...
            if (isEditing && editingJustification?.id) {
                ...
                await updateDoc(docRef, justificationData);
            } else {
            ...
            toast({ 
                title: `Justificativa ${isEditing ? 'atualizada' : 'adicionada'}!`, 
                description: `A justificativa "${formData.name}" foi salva com sucesso.` 
            });
            setIsDialogOpen(false);
        } catch (error: any) {
            console.error(error);
            toast({ variant: "destructive", title: "Erro", description: "Falha ao salvar os dados." });
        }
    };
    ...
    const handleDeleteJustificationConfirm = async () => {
        ...
        try {
            ...
            toast({ title: "Justificativa removida!", description: `A justificativa "${deletingJustification.name}" foi removida.` });
        } catch (error: any) {
    ...
    <>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2"><ClipboardCheck /> Justificativas</CardTitle>
            <CardDescription>Gerencie os motivos para abonos e ajustes de ponto.</CardDescription>
          </div>
                      ...
                      {!justifications || justifications.length === 0 ? (
                        ...
                        <TableRow key={justification.id}>
                            ...
                            <TableCell className="text-right">
                                ...
                                <Button variant="ghost" size="icon" className="hover:text-destructive" onClick={() => (setDeletingJustification(justification), setIsConfirmDeleteDialogOpen(true))}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <JustificationForm 
            ...
            onSubmit={handleSubmitSector} // Certifique-se que o componente JustificationForm aceita o onSubmit
        />
      </Dialog>
      ...
      <AlertDialog open={isConfirmDeleteDialogOpen} onOpenChange={setIsConfirmDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza que deseja remover &quot;{deletingJustification?.name}&quot;? 
              Ajustes de ponto que já utilizam esta justificativa podem ser afetados.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={handleDeleteJustificationConfirm}>Confirmar</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
